// ============================================
// G-KODE - COMPLETE APP WITH SUPABASE
...
function getCompaniesLocal() {
    try { return JSON.parse(localStorage.getItem('gkode_companies') || '[]'); } catch { return []; }
}
...
function getProductsLocal() {
    try { return JSON.parse(localStorage.getItem('gkode_products') || '[]'); } catch { return []; }
}
...
function getProfessions() {
    try { return JSON.parse(localStorage.getItem('gkode_professions') || '[]'); } catch { return []; }
}
...
function getCategories() {
    try { return JSON.parse(localStorage.getItem('gkode_categories') || '[]'); } catch { return []; }
}
...
function showScreen(id) {
    ...
    const allowed = ['home', 'postGig', 'profile', 'chat', 'marketplace', 'companyRegister', 'companyDashboard', 'addProduct', 'complaint'];
...
function saveNewProfession(professionName) {
    ...
    const formatted = professionName.trim().split(' ').map(w => w.charAt(0).toUpperCase() + w.slice(1).toLowerCase()).join(' ');
    const all = getAllProfessions();
...
function getAllCategories() {
    ...
    saved.forEach(c => { if (!all.includes(c)) all.push(c); });
    all.sort();
    return all;
}
...
function sendOTPEmail(email, name, code) {
    loadEmailJS(function() {
        ...
        emailjs.send(EMAILJS_CONFIG.serviceID, EMAILJS_CONFIG.otpTemplateID, templateParams)
            .then(() => showToast('📧 Verification code sent to your email!', 'success'))
...
async function register(e) {
    ...
    try {
        ...
        if (!name || !phone || !id || !email || !password || !location || !profession) {
            ...
            btn.disabled = false; btn.textContent = 'REGISTER'; isProcessing = false; return;
        }
        ...
        // Create user object
        const user = {
            name, phone, id, email, password, location,
            profession: finalProfession, skills: skills || '',
            photo: photoData, idScan: idData,
            isPaid: false, isBanned: false, strikes: 0, rating: 0, reviewCount: 0,
        ...
        btn.disabled = false; btn.textContent = 'REGISTER'; isProcessing = false;
    }
}
...
async function completeRegistration() {
    ...
    try {
        ...
        showToast(saved ? '✅ User saved to cloud!' : '✅ Saved locally!', 'success');
        showScreen('home');
        loadGigs();
        updateBottomNav();
...
async function login(e) {
    ...
    try {
        ...
        if (!phone || !password) {
            ...
            btn.disabled = false; btn.textContent = 'LOGIN'; isProcessing = false; return;
        }

        // Try Supabase first
        let userData = await findUserByPhone(phone);
        ...
        if (userData) {
            if (userData.password_hash === password) {
                ...
                loadGigs();
                updateBottomNav();
                btn.disabled = false; btn.textContent = 'LOGIN'; isProcessing = false;
                return;
            }
        }
...
function postGig(e) {
    ...
    try {
        ...
        if (!title || !skill || !location || !budgetMin || !budgetMax) {
            ...
            btn.disabled = false; btn.textContent = 'POST GIG'; isProcessing = false; return;
        }
        ...
        btn.disabled = false; btn.textContent = 'POST GIG'; isProcessing = false;
    }
}
...
function loadGigs() {
    ...
    filtered.forEach(g => {
        ...
        const urgencyColor = g.urgency === 'Emergency' ? '#cc0000' :
                            g.urgency === 'Urgent' ? '#ff9800' : '#006400';
        ...
        html += `
            <div class="gig-card" style="border-left:4px solid ${urgencyColor};">
                ...
                ${open ? `<div class="gig-actions"><button class="btn-accept" onclick="acceptGig('${g.id}')">✅ ACCEPT</button></div>` : 
...
function loadProfile() {
    ...
    document.getElementById('profileStatus').innerHTML = `${statusText} | ⭐ ${currentUser.rating || 0} (${currentUser.reviewCount || 0} reviews)`;
...
function registerCompany(e) {
    ...
    try {
        ...
        if (!name || !type || !regNo || !location || !phone) {
            showToast('Fill all required fields.', 'error');
            btn.disabled = false; btn.textContent = 'REGISTER BUSINESS'; isProcessing = false; return;
        }
        ...
        companies.push({
            id: Date.now().toString(),
            name, type, regNo, location, phone, email, desc,
        ...
        btn.disabled = false; btn.textContent = 'REGISTER BUSINESS'; isProcessing = false;
    }
}
...
function addProduct(e) {
    ...
    try {
        ...
        if (!name || !category || !unit || !price || !stock) {
            ...
            btn.disabled = false; btn.textContent = 'ADD PRODUCT'; isProcessing = false; return;
        }
        ...
        btn.disabled = false; btn.textContent = 'ADD PRODUCT'; isProcessing = false;
    }
}
...
function submitReview() {
    ...
    let reviews = JSON.parse(localStorage.getItem('gkode_reviews') || '[]');
    reviews.push({
        id: Date.now().toString(),
    ...
    localStorage.setItem('gkode_reviews', JSON.stringify(reviews));
...
function exportUserData() {
    ...
    const data = {
        ...
        user: currentUser,
        userGigs: getGigsLocal().filter(g => g.client === currentUser.name || g.worker === currentUser.name),
        userOrders: [],
        userPayments: JSON.parse(localStorage.getItem('gkode_payments') || '[]').filter(p => p.phone === currentUser.phone)
    };
...
function deleteAccount() {
    ...
    let gigs = getGigsLocal().filter(g => g.client !== currentUser.name && g.worker !== currentUser.name);
    setGigsLocal(gigs);
